<?php
    ...
    class LinkPager extends LP
    {
        ...
        protected function renderPageButtons()
        {
           ...
           /* // first page
            ...
            if ($this->prevPageLabel !== false && $currentPage>0) {
                ...
                $buttons_prev = $this->renderAPageButton($this->prevPageLabel, $page, 'paginator-prev', $currentPage <= 0, false);



               /*if($currentPage>1)
                    $buttons[] = $this->renderPageButton(1, 0, null, false, false).(($currentPage>2)?'<li>...</li>':'');*/
            }
            ...
            if ($this->nextPageLabel !== false  && $currentPage<( $pageCount - 1 ) ) {
                ...
                $buttons_next = $this->renderAPageButton($this->nextPageLabel, $page, 'paginator-next', $currentPage >= $pageCount - 1, false);
            }

            // last page
            $lastPageLabel = $this->lastPageLabel === true ? $pageCount : $this->lastPageLabel;
            if ($lastPageLabel !== false && $pageCount>3) {
                ...
                if($currentPage<($pageCount - 2))
                    ...
                    if($currentPage >= ($pageCount - 3) &&  $currentPage!= $pageCount)
                         ...
                         $buttons[] = '<li>...</li>'.$this->renderPageButton($lastPageLabel, $pageCount - 1, $this->lastPageCssClass, $currentPage >= $pageCount - 1, false);
            }

            return $buttons_prev.Html::tag('ul', implode("\n", $buttons), $this->options).$buttons_next;
        }
        ...
        protected function renderAPageButton($label, $page, $class, $disabled, $active)
        {
            $options = ['class' =>  $class];
            /*if ($active) {
                Html::addCssClass($options, $this->activePageCssClass);
