...
class Catalog extends AR
{
      ...
      return [
         ...
         /* [
          ...
          [
              ...
              'table' => 'catalog',
              'index' => 'catalog_id',
              'fields' =>['catalog_bids','iszoomer','catalog_public','catalog_url','catalog_date','catalog_dateend','catalog_bonus', 'catalogcatid', 'catalog_price', 'catalog_price_step','timeend','catalog_count' ]
          ],
                     'Character' => [
	   ...
	   public function rules()
    {
        ...
        return [
            ...
            [[ 'catalog_meta','catalog_keys','catalog_title','catalog_url','catalog_price_step'], 'default'],
...
public function getBonus(){
    ...
    public function priceBuy(){
        return $this->getPrice(  $this->catalog_price );
    }
    public function getPriceOne(){
        $priceAuct=\app\models\Auct::priceBuy($this->catalog_id);
        if($priceAuct)
            return $priceAuct+$this->catalog_price*$this->catalog_price_step/100;
        else
    ...
    public function persentToEnd(){
       ...
       /*  echo $start.' '.$this->catalog_date.'<br>';
        echo $now.'<br>';
        echo $end.' '.$this->catalog_dateend.'<br>';*/

        $result=(100*$partEnd)/$all;
        //echo $result;
        if($result > 0){
            $result = str_replace('.',',',floor (100-round($result ,0,PHP_ROUND_HALF_DOWN)));
            //$result=100-round($result ,0,PHP_ROUND_HALF_DOWN);
          //  echo $result."<br>";
            return $result;
        }
        else
    ...
    public function decriment(){
        $field='catalog_count';
        $value=$this->catalog_count--;
        $id=$this->catalog_id;
        \yii::$app->db->createCommand()->update('catalog',[ $field => $value ],[ 'catalog_id' => $id ])->execute();
        \yii::$app->db->createCommand()->update('catalog',[ $field => $value ],[ 'langid' => $id ])->execute();
    }

    public function nextDateEnd(){
        $rand=[0.33,1,2,3];
        if ($this->catalog_count > 0) {
            $where['catalog_date']=date('Y-m-d H:i:s');
            $where['timeend']=$rand[rand(0,(count($rand)-1))];
            $where['catalog_dateend']=date('Y-m-d H:i:s',mktime()+ $where['timeend']/*$this->timeend*/*24*3600);
        }else{
            $where['catalog_count']=0;
            $where['catalog_public']=0;
        }
        \yii::$app->db->createCommand()->update('catalog',$where,[ 'catalog_id' => $this->catalog_id ])->execute();
        \yii::$app->db->createCommand()->update('catalog',$where,[ 'langid' => $this->catalog_id ])->execute();
    }

    public function savePersent(){

        $where['persent']=$this->persentToEnd();
        \yii::$app->db->createCommand()->update('catalog',$where,[ 'catalog_id' => $this->catalog_id ])->execute();
        \yii::$app->db->createCommand()->update('catalog',$where,[ 'langid' => $this->catalog_id ])->execute();
    }
